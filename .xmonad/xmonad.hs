...
myWorkspaces = [wsDEV, wsCODE, wsTERM, wsWORK, wsFILE, wsMON, wsPERS, wsTMP, wsTMP, wsSYS]
...
myRedshiftToggle    = "if [ $(systemctl --user is-active redshift) = active ] ; then systemctl --user stop redshift ; else systemctl --user start redshift; fi"
...
hangoutsTitle     = "Google Hangouts - sergeylarionov@cloud.upwork.com"
...
isPersonalHangouts  = isHangoutsFor "sergeylarionov"
isWorkHangouts      = isHangoutsFor "sergey.larionov"
...
--    , ("M-S-/"                  , addName "On-screen keys settings"         $ spawn "screenkey --show-settings")
    ...
    , ("M1-S-p"                 , addName "Capture screen - area select"    $ spawn "flameshot gui" )
...
--    , ("M1-x"                   , addName "NSP Xawtv"                       $ namedScratchpadAction scratchpads "xawtv")
    ...
    , ("M-M1-r"                    , addName "Toggle Redshift"                 $ spawn myRedshiftToggle)
    ] ^++^
    ...
    , ("M-S-q"        , addName "Kill all"                        $ confirmPrompt hotPromptTheme "kill all" $ killAll)
...
myStartupHook = do
    ...
    -- spawn "/home/larionov/bin/wm/init-wallpaper"
